...
KEYRGB_BOOTSTRAP_REF="${KEYRGB_BOOTSTRAP_REF:-v0.32.0}"
...
Bootstrap (curl installs):
    --ref <git-ref>    Git ref for downloading scripts/ from GitHub raw (default: v0.32.0)
...
bootstrap_and_run() {
    ...
    case "$KEYRGB_BOOTSTRAP_REF" in
        main|master|HEAD|develop)
            ...
            echo "   curl -fsSL https://raw.githubusercontent.com/${KEYRGB_REPO_OWNER}/${KEYRGB_REPO_NAME}/v0.32.0/install.sh | bash" >&2
            ;;
    esac
