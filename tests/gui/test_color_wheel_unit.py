...
def test_draw_wheel_uses_existing_cache_file(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(color_wheel_module, "build_wheel_ppm_bytes", lambda **_kwargs: (_ for _ in ()).throw(AssertionError()))
    monkeypatch.setattr(color_wheel_module, "write_bytes_atomic", lambda *_args, **_kwargs: (_ for _ in ()).throw(AssertionError()))
