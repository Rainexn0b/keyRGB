...
@pytest.mark.parametrize(
    "config_error",
    [RuntimeError("boom"), color_wheel_ui.tk.TclError("boom")],
)
def test_set_brightness_label_text_falls_back_and_swallows_label_errors(
    raw_text: str | None,
    config_error: Exception,
) -> None:
    wheel = _ColorWheelHarness()
    wheel.brightness_title_label = _FakeConfigurable(config_error=config_error)
...
def test_set_brightness_label_text_propagates_unexpected_label_errors() -> None:
    wheel = _ColorWheelHarness()
    wheel.brightness_title_label = _FakeConfigurable(config_error=ValueError("boom"))
...
def test_update_preview_refreshes_canvas_label_and_manual_vars() -> None:
    ...
    assert wheel.preview_canvas.rectangle_calls == [((0, 0, 200, 30), {"fill": "#0c2238", "outline": ""})]
...
def test_update_preview_respects_rgb_entry_sync_guard() -> None:
    ...
    assert wheel.preview_canvas.delete_calls == ["all"]
    assert wheel.preview_canvas.rectangle_calls == [((0, 0, 200, 30), {"fill": "#070809", "outline": ""})]
...
def test_on_manual_rgb_set_propagates_unexpected_parse_errors() -> None:
    wheel = _ColorWheelHarness(current_value=0.375)
    wheel.rgb_r_var = _FakeVar(_BrokenStrValue())
    wheel.rgb_g_var = _FakeVar("20")
    wheel.rgb_b_var = _FakeVar("30")

    with pytest.raises(RuntimeError, match="broken __str__"):
        wheel._on_manual_rgb_set()

    assert wheel.set_color_calls == []
    assert wheel.invoke_callback_calls == []
