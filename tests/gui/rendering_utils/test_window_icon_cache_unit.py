...
def test_load_window_icon_image_falls_back_to_gdkpixbuf_svg_rasterizer(tmp_path, monkeypatch) -> None:
    window_icon.clear_cached_window_icon_images()
    image_path = tmp_path / "icon.svg"
    image_path.write_text("<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 1 1'></svg>", encoding="utf-8")
    ...
    def fake_gdkpixbuf(path_str: str):
        assert path_str == str(image_path)
        out = BytesIO()
        Image.new("RGBA", (64, 64), color=(9, 8, 7, 255)).save(out, format="PNG")
        with Image.open(BytesIO(out.getvalue())) as image:
            return image.convert("RGBA")

    monkeypatch.setattr(window_icon, "_rasterize_svg_window_icon_with_gdkpixbuf", fake_gdkpixbuf)

    image = window_icon.load_window_icon_image(image_path)

    assert image.size == (64, 64)
    assert image.getpixel((0, 0)) == (9, 8, 7, 255)

    window_icon.clear_cached_window_icon_images()


def test_find_keyrgb_logo_path_keeps_repo_icon_when_home_lookup_fails(tmp_path, monkeypatch) -> None:
    repo_icon = tmp_path / "repo-icon.png"
    _save_icon(repo_icon, (12, 34, 56, 255))

    monkeypatch.setattr(window_icon, "_repo_candidate_logo_paths", lambda: [repo_icon])
    monkeypatch.setattr(window_icon.Path, "home", lambda: (_ for _ in ()).throw(RuntimeError("no home")))
...
def test_apply_keyrgb_window_icon_swallows_unexpected_window_callback_failure(tmp_path, monkeypatch) -> None:
    ...
    class BrokenWindow:
        def iconphoto(self, _default: bool, _photo: object) -> None:
            raise LookupError("broken tk callback")
    ...
    monkeypatch.setattr(window_icon.logger, "exception", lambda message, *, exc_info: logs.append((message, exc_info)))

    window = BrokenWindow()
    window_icon.apply_keyrgb_window_icon(window)

    assert window.keyrgb_icon_image["image"].getpixel((0, 0)) == (255, 0, 0, 255)
    assert len(logs) == 1
