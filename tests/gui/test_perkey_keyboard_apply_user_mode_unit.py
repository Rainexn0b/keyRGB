...
def test_push_per_key_colors_handles_oserror_device_states(
    ...
    class DummyKb:
        def set_key_colors(self, colors, *, brightness: int, enable_user_mode: bool = True):
            raise OSError("device issue")

    kb = DummyKb()
    monkeypatch.setattr(keyboard_apply, "is_device_busy", lambda exc: busy)
    monkeypatch.setattr(keyboard_apply, "is_device_disconnected", lambda exc: disconnected)
...
def test_push_per_key_colors_returns_keyboard_for_unexpected_exceptions() -> None:
    class DummyKb:
        def set_key_colors(self, colors, *, brightness: int, enable_user_mode: bool = True):
            raise RuntimeError("boom")
