...
from keyrgb.core.backends.base import DEFAULT_BACKEND_CAPABILITIES, BackendCapabilities
...
def test_select_backend_with_introspection_happy_path() -> None:
    from keyrgb.tray.app.backend import select_backend_with_introspection

    backend = MagicMock()
    backend.probe.return_value = {"ok": True}
    backend.capabilities.return_value = {"per_key": False, "brightness": True}

    with patch("keyrgb.tray.app.backend.select_backend", return_value=backend):
...
def test_load_ite_dimensions_falls_back_and_logs_traceback() -> None:
    from keyrgb.tray.app.backend import load_ite_dimensions

    logger = MagicMock()

    with (
        patch("keyrgb.tray.app.backend.logging.getLogger", return_value=logger),
        patch("keyrgb.tray.app.backend.select_backend", side_effect=RuntimeError("boom")),
    ):
...
def test_load_ite_dimensions_propagates_unexpected_errors() -> None:
    ...
    with (
        patch("keyrgb.tray.app.backend.select_backend", side_effect=AssertionError("unexpected backend bug")),
        pytest.raises(AssertionError, match="unexpected backend bug"),
    ):
        load_ite_dimensions()
...
def test_select_backend_with_introspection_handles_probe_exception() -> None:
    ...
    with (
        patch("keyrgb.tray.app.backend.logging.getLogger", return_value=logger),
        patch("keyrgb.tray.app.backend.select_backend", return_value=backend),
    ):
        b, probe, caps = select_backend_with_introspection()
...
def test_select_backend_with_introspection_propagates_unexpected_probe_exception() -> None:
    ...
    with (
        patch("keyrgb.tray.app.backend.select_backend", return_value=backend),
        pytest.raises(AssertionError, match="unexpected probe bug"),
    ):
...
def test_select_backend_with_introspection_propagates_unexpected_caps_exception() -> None:
    ...
    with (
        patch("keyrgb.tray.app.backend.select_backend", return_value=backend),
        pytest.raises(AssertionError, match="unexpected caps bug"),
    ):
        select_backend_with_introspection()
...
def test_select_backend_with_introspection_handles_none_backend() -> None:
    ...
    with patch("keyrgb.tray.app.backend.select_backend", return_value=None):
        b, probe, caps = select_backend_with_introspection()
...
def test_select_device_discovery_snapshot_returns_payload() -> None:
    from keyrgb.tray.app.backend import select_device_discovery_snapshot

    payload = {"candidates": [{"device_type": "lightbar"}]}

    with patch("keyrgb.tray.app.backend.collect_device_discovery", return_value=payload):
...
def test_select_device_discovery_snapshot_swallows_errors() -> None:
    ...
    with (
        patch("keyrgb.tray.app.backend.logging.getLogger", return_value=logger),
        patch("keyrgb.tray.app.backend.collect_device_discovery", side_effect=RuntimeError("boom")),
    ):
...
def test_select_device_discovery_snapshot_propagates_unexpected_errors() -> None:
    ...
    with (
        patch(
            "keyrgb.tray.app.backend.collect_device_discovery",
            side_effect=AssertionError("unexpected discovery bug"),
        ),
        pytest.raises(AssertionError, match="unexpected discovery bug"),
    ):
