...
import src.tray.pollers.idle_power.sensors as sensors
...
def test_read_screen_off_state_drm_skips_connector_with_unreadable_status(tmp_path, monkeypatch):
    drm = tmp_path / "sys" / "class" / "drm"
...
def test_run_returns_none_on_subprocess_failure(monkeypatch):
    def fake_run(*args, **kwargs):
        raise subprocess.TimeoutExpired(cmd=["echo", "hi"], timeout=1.0)
