...
def test_secondary_signature_change_is_not_misclassified_as_base_or_target_change() -> None:
    ...
    assert classify_fast_path_change(last_applied=_state(secondary_sig=(("a", ()),)), current=_state(secondary_sig=(("b", ()),))) == "secondary_only"
...
def test_normal_uniform_apply_restores_static_scene_regardless_of_effect_output() -> None:
    ...
    with patch(
        "src.tray.pollers.config_polling_internal.helpers._apply_secondary_static_from_config"
    ) as apply_static:
