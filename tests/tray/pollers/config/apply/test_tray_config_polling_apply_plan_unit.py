...
def test_apply_mode_is_classified_on_plan() -> None:
    assert (
        classify_config_apply_plan(configured_effect="perkey", current=_mk_state(effect="perkey")).apply_mode
        == "perkey"
    )
    assert (
        classify_config_apply_plan(configured_effect="none", current=_mk_state(effect="none")).apply_mode == "uniform"
    )
    assert (
        classify_config_apply_plan(
