...
def test_get_engine_color_map_buffer_bypasses_hostile_engine_attribute_lookup() -> None:
    ...
    class _HostileEngine:
        def __getattribute__(self, name: str):
            if name == "_sw_frame_map":
                raise RuntimeError("hostile attribute lookup")
            return object.__getattribute__(self, name)

    engine = _HostileEngine()

    first = get_engine_color_map_buffer(engine, "_sw_frame_map")
    second = get_engine_color_map_buffer(engine, "_sw_frame_map")

    assert first == {}
    assert second is first
    assert vars(engine)["_sw_frame_map"] is first
...
def test_get_engine_color_map_buffer_reraises_unexpected_assignment_failures() -> None:
    ...
    class _BrokenSlotEngine:
        ...
        def __setattr__(self, name: str, value: object) -> None:
            raise RuntimeError(f"unexpected failure assigning {name}={value!r}")

    with pytest.raises(RuntimeError, match="unexpected failure assigning _sw_frame_map"):
        get_engine_color_map_buffer(_BrokenSlotEngine(), "_sw_frame_map")
