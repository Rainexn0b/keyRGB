...
def test_backend_report_delay_env_key_normalizes_backend_names() -> None:
    ...
    assert backend_report_delay_env_key("ite8258_chassis") == "KEYRGB_ITE8258_CHASSIS_REPORT_DELAY_MS"
    assert backend_report_delay_env_key("ite8295_zones") == "KEYRGB_ITE8295_ZONES_REPORT_DELAY_MS"
...
def test_backend_specific_delay_overrides_global(monkeypatch) -> None:
    ...
    assert hid_report_delay_s_from_env(backend_name="ite8258_chassis") == 0.0025
...
def test_invalid_backend_specific_delay_falls_back_to_global(monkeypatch) -> None:
    ...
    assert hid_report_delay_s_from_env(backend_name="ite8258_chassis") == 0.004
...
def test_sleep_after_hid_report_allows_zero_to_disable(monkeypatch) -> None:
    ...
    sleep_after_hid_report(backend_name="ite8258_chassis")
