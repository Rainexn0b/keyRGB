...
def test_backend_report_delay_env_key_normalizes_backend_names() -> None:
    assert backend_report_delay_env_key("ite8291r3") == "KEYRGB_ITE8291R3_REPORT_DELAY_MS"
    assert backend_report_delay_env_key("ite8258-chassis") == "KEYRGB_ITE8258_CHASSIS_REPORT_DELAY_MS"
    assert backend_report_delay_env_key("ite8295-zones") == "KEYRGB_ITE8295_ZONES_REPORT_DELAY_MS"
...
def test_backend_specific_delay_overrides_global(monkeypatch) -> None:
    ...
    monkeypatch.setenv("KEYRGB_ITE8258_CHASSIS_REPORT_DELAY_MS", "2.5")

    assert hid_report_delay_s_from_env(backend_name="ite8258-chassis") == 0.0025
...
def test_invalid_backend_specific_delay_falls_back_to_global(monkeypatch) -> None:
    ...
    monkeypatch.setenv("KEYRGB_ITE8258_CHASSIS_REPORT_DELAY_MS", "not-a-number")

    assert hid_report_delay_s_from_env(backend_name="ite8258-chassis") == 0.004


def test_sleep_after_hid_report_allows_zero_to_disable(monkeypatch) -> None:
    sleeps: list[float] = []
    monkeypatch.setenv("KEYRGB_ITE8258_CHASSIS_REPORT_DELAY_MS", "0")
    monkeypatch.setattr("src.core.backends._report_pacing.time.sleep", sleeps.append)

    sleep_after_hid_report(backend_name="ite8258-chassis")
