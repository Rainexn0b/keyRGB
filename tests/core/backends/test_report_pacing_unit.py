...
def test_backend_specific_delay_overrides_global(monkeypatch) -> None:
    ...
    monkeypatch.setenv("KEYRGB_ITE8258_PERKEY_CHASSIS_REPORT_DELAY_MS", "2.5")

    assert hid_report_delay_s_from_env(backend_name="ite8258_perkey_chassis") == 0.0025
...
def test_invalid_backend_specific_delay_falls_back_to_global(monkeypatch) -> None:
    ...
    monkeypatch.setenv("KEYRGB_ITE8258_PERKEY_CHASSIS_REPORT_DELAY_MS", "not-a-number")

    assert hid_report_delay_s_from_env(backend_name="ite8258_perkey_chassis") == 0.004
