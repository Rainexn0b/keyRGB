...
from keyrgb.core.backends.ite8295_zones_lenovo_ideapad import protocol
from keyrgb.core.backends.ite8295_zones_lenovo_ideapad.backend import (
    Ite8295ZonesBackend,
    _find_matching_supported_hidraw_device,
    _open_matching_transport,
)
from keyrgb.core.backends.ite8295_zones_lenovo_ideapad.device import Ite8295ZonesKeyboardDevice
...
def test_backend_probe_reports_unavailable_when_no_matching_device(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "keyrgb.core.backends.ite8295_zones_lenovo_ideapad.backend._find_matching_supported_hidraw_device",
        lambda: None,
    )
...
def test_open_matching_transport_raises_when_no_supported_device(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "keyrgb.core.backends.ite8295_zones_lenovo_ideapad.backend.find_matching_hidraw_device", lambda **kwargs: None
    )
...
def test_backend_get_device_wraps_permission_error(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "keyrgb.core.backends.ite8295_zones_lenovo_ideapad.backend._open_matching_transport",
