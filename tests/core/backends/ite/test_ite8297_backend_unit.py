...
from keyrgb.core.backends.ite8297_uniform import protocol
from keyrgb.core.backends.ite8297_uniform.backend import (
    Ite8297Backend,
...
def test_backend_probe_reports_unavailable_when_no_matching_device(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "keyrgb.core.backends.ite8297_uniform.backend._find_matching_supported_hidraw_device", lambda: None
    )
...
def test_find_matching_supported_hidraw_device_falls_back_to_scanned_match_when_forced_missing(
    ...
    monkeypatch.setattr(
        "keyrgb.core.backends.ite8297_uniform.backend.find_matching_hidraw_device", lambda *_a, **_k: dummy_match
    )
...
def test_backend_probe_reports_detected_but_disabled_until_opted_in(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "keyrgb.core.backends.ite8297_uniform.backend._find_matching_supported_hidraw_device", lambda: DummyMatch()
    )
...
def test_backend_get_device_wraps_permission_error(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "keyrgb.core.backends.ite8297_uniform.backend._open_matching_transport", lambda: (_ for _ in ()).throw(err)
    )
...
def test_backend_get_device_propagates_unexpected_open_errors(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("KEYRGB_ENABLE_EXPERIMENTAL_BACKENDS", "1")
    monkeypatch.setattr(
        "keyrgb.core.backends.ite8297_uniform.backend._open_matching_transport",
