...
class TestLayoutGlobalLoadSave:
    ...
    def test_load_layout_global_uses_layout_specific_default_if_missing(self, temp_profile_dir, profile_paths_factory, monkeypatch):
        from src.core.profile import profiles
        ...
        monkeypatch.setattr(
            ...
            lambda layout_id=None: {"dx": 2.0, "dy": 3.0, "sx": 1.0, "sy": 1.0, "inset": 0.06},
        )

        loaded = profiles.load_layout_global("test_profile", physical_layout="iso")
        assert loaded == {"dx": 2.0, "dy": 3.0, "sx": 1.0, "sy": 1.0, "inset": 0.06}
