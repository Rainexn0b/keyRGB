...
import src.core.diagnostics.additional_evidence as evidence
...
def test_collect_additional_evidence_runs_direct_and_privileged(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        evidence.shutil,
        "which",
        lambda name: {"lsusb": "/usr/bin/lsusb", "usbhid-dump": "/usr/bin/usbhid-dump", "pkexec": "/usr/bin/pkexec"}.get(name),
    )
