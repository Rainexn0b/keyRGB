from __future__ import annotations

import json

import pytest

from tests._paths import ensure_repo_root_on_sys_path


ensure_repo_root_on_sys_path()

import src.core.power.tcc_profiles as tcc_power_profiles
from src.core.power.tcc_profiles import json_api as tcc_json_api
from src.core.power.tcc_profiles.busctl import (
    _parse_busctl_string_reply,
    _parse_busctl_bool_reply,
)


@pytest.mark.parametrize(
    "stdout, expected",
    [
        ('s "{\\"a\\": 1}"', '{"a": 1}'),
        ('s "hello"', "hello"),
        ("", None),
        ("b true", None),
    ],
)
def test_parse_busctl_string_reply(stdout: str, expected: str | None) -> None:
    assert _parse_busctl_string_reply(stdout) == expected


@pytest.mark.parametrize(
    "stdout, expected",
    [
        ("b true", True),
        ("b false", False),
        ("b 1", True),
        ("b 0", False),
        ('s "x"', None),
        ("", None),
    ],
)
def test_parse_busctl_bool_reply(stdout: str, expected: bool | None) -> None:
    assert _parse_busctl_bool_reply(stdout) == expected


def test_update_custom_profile_preserves_id_and_strips_name(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    stored: dict[str, object] = {}

    def fake_load_custom_profiles_payload():
        return [
            {"id": "abc", "name": "Old", "x": 1},
            {"id": "def", "name": "Other"},
        ]

    def fake_write_temp_json(payload, *, prefix: str):
        # validate payload is JSON-serializable and capture it
        json.dumps(payload)
        stored["payload"] = payload
        stored["prefix"] = prefix
        return "/tmp/fake.json"

    def fake_apply_new_profiles_file(path: str) -> None:
        stored["applied"] = path

    monkeypatch.setattr(
        tcc_power_profiles,
        "_load_custom_profiles_payload",
        fake_load_custom_profiles_payload,
    )
    monkeypatch.setattr(tcc_power_profiles, "_write_temp_json", fake_write_temp_json)
    monkeypatch.setattr(tcc_power_profiles, "_apply_new_profiles_file", fake_apply_new_profiles_file)

    tcc_power_profiles.update_custom_profile("abc", {"id": "zzz", "name": "  New Name  ", "x": 2})

    assert stored["applied"] == "/tmp/fake.json"
    payload = stored["payload"]
    assert isinstance(payload, list)
    # First profile updated
    assert payload[0]["id"] == "abc"
    assert payload[0]["name"] == "New Name"
    assert payload[0]["x"] == 2
    # Second profile unchanged
    assert payload[1]["id"] == "def"


def test_update_custom_profile_rejects_empty_name(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        tcc_power_profiles,
        "_load_custom_profiles_payload",
        lambda: [{"id": "abc", "name": "Old"}],
    )

    with pytest.raises(tcc_power_profiles.TccProfileWriteError):
        tcc_power_profiles.update_custom_profile("abc", {"name": "   "})


def test_get_custom_profile_payload_returns_detached_copy(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    payload = {"id": "abc", "name": "Old", "nested": {"x": 1}}

    monkeypatch.setattr(
        tcc_power_profiles,
        "_load_custom_profiles_payload",
        lambda: [payload],
    )

    result = tcc_power_profiles.get_custom_profile_payload("abc")

    assert result == payload
    assert result is not payload
    assert result is not None
    assert result["nested"] is not payload["nested"]


def test_list_profiles_returns_empty_list_for_malformed_json(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(tcc_json_api, "get_profiles_json", lambda: "{")

    assert tcc_json_api.list_profiles() == []


def test_list_profiles_degrades_to_empty_list_for_non_string_json_payload() -> None:
    assert tcc_json_api._list_profiles_from_json(object()) == []


def test_list_profiles_propagates_unexpected_loader_failures(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(tcc_json_api.json, "loads", lambda _raw: (_ for _ in ()).throw(RuntimeError("boom")))

    with pytest.raises(RuntimeError, match="boom"):
        tcc_json_api._list_profiles_from_json("[]")


def test_list_profiles_skips_malformed_entries_and_keeps_valid_profiles(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    class BrokenDescription:
        def __str__(self) -> str:
            raise RuntimeError("broken description")

    payload = [
        {"id": "balanced", "name": "Balanced", "description": "Default profile"},
        {"id": "missing-name"},
        ["not-a-dict"],
        {"id": "eco", "name": "Eco", "description": None},
        {"id": "broken", "name": "Broken", "description": BrokenDescription()},
    ]
    monkeypatch.setattr(tcc_json_api.json, "loads", lambda _raw: payload)

    assert tcc_json_api._list_profiles_from_json("[]") == [
        tcc_power_profiles.TccProfile(id="balanced", name="Balanced", description="Default profile"),
        tcc_power_profiles.TccProfile(id="eco", name="Eco", description=""),
    ]


def test_list_profiles_round_trips_valid_entries() -> None:
    raw = json.dumps(
        [
            {"id": "balanced", "name": "Balanced", "description": "Default profile"},
            {"id": "turbo", "name": "Turbo", "description": 123},
        ]
    )

    assert tcc_json_api._list_profiles_from_json(raw) == [
        tcc_power_profiles.TccProfile(id="balanced", name="Balanced", description="Default profile"),
        tcc_power_profiles.TccProfile(id="turbo", name="Turbo", description="123"),
    ]
