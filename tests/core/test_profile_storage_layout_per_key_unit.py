...
class TestLayoutPerKeyLoadSave:
    ...
    def test_load_per_key_uses_layout_specific_default_if_missing(self, temp_profile_dir, profile_paths_factory, monkeypatch):
        from src.core.profile import profiles

        def mock_paths(_name):
            return profile_paths_factory(temp_profile_dir, layout_per_key=temp_profile_dir / "nonexistent_layout_per_key.json")
