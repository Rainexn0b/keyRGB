...
|---|---|---|
| mypy | yes | `src/core`, `src/tray`, `buildpython`, `scripts/release`, `tests/buildpython`, plus a narrow non-Tk GUI baseline. `warn_unused_ignores`, `warn_redundant_casts`, and `no_implicit_optional` are on. Tk-heavy GUI is still excluded. |
...
| Architecture rules | yes | Configured warning and error findings both fail Step 17. `src/gui/perkey/hardware.py` is the per-key hardware bootstrap and is excluded from the backend-selection rule. |
