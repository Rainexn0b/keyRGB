...
|---|---|---|
| mypy | yes | `keyrgb/core`, `keyrgb/tray`, `keyrgb/gui`, `buildpython`, `scripts/release`, `scripts/dependency_audit.py`, `tests/buildpython`. Typed against the 3.10 floor with explicit package bases so namespace-style test helpers have one module identity. `warn_unused_ignores`, `warn_redundant_casts`, and `no_implicit_optional` are on. Unrelated host stub packages that KeyRGB does not import (`numpy`) are skipped so newer stub syntax cannot fail the gate. |
...
| Architecture rules | yes | Configured warning and error findings both fail Step 17. `keyrgb/gui/perkey/hardware.py` is the per-key hardware bootstrap and is excluded from the backend-selection rule. Composite ITE 8258 coordinator/protocol imports stay inside that backend package; pollers must not load pystray; core profile activation cannot import tray or look up private tray names; hardware-effect builders cannot inspect `__code__`/`__closure__`. |
