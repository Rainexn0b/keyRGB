...
|---:|---|---|:---:|:---:|---|---|
| 1 | Diagnostics | Split `src/core/diagnostics.py` into collectors + formatting module | P0 | M | done | Highest LOC and touches many support workflows. |
| 2 | Settings UI | Extract scrollframe + diagnostics panel from `src/gui/power.py` | P0 | M | done | Keep behavior identical; reduce single-file complexity. |
...
| 5 | Tray | Reduce responsibility in `src/tray/app.py` (delegate to services) | P2 | M | done | Mostly cleanup; avoid feature churn. |
