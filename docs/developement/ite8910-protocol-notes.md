...
`((row & 0x07) << 5) | (col & 0x1F)` - 6 rows, 20 columns.

## Command sequence

Animation modes:
1. `[CC, 00, mode_id]` - activate mode
2. `[CC, 15/16/17, slot, R, G, B]` - set color/direction (if applicable)
3. `[CC, 09, brightness, speed]` - set brightness and speed

Per-key (Direct) mode:
1. `[CC, 00, 0C]` - clear all LEDs
2. `[CC, 09, brightness, speed]` - set brightness
