...
An independent research report (`docs/genAI/research-device-support/deep-research-report.md`) compared KeyRGB's implementation against a separate, fully-working Linux implementation tested on the same `83F5` hardware platform. That comparison revealed **three confirmed discrepancies** and **one suspected discrepancy** in the protocol layer. The keyboard path works, but these discrepancies must be resolved before chassis-zone support (logo, neon, vents) can be considered reliable.

This document is the authoritative correction plan. All fixes described here must land **before** the secondary-device route refactor (`docs/developement/architecture/secondary-device-route-refactor-plan.md`) proceeds.
...
**Current KeyRGB (`src/core/backends/ite8258_chassis/protocol.py`):**
```python
DIRECTION_UP = 0x01
DIRECTION_DOWN = 0x02
DIRECTION_LEFT = 0x03
...
**Clarification:** KeyRGB has full UI-level layout support. The per-key editor supports ANSI, ISO, KS, ABNT, and JIS visual layouts, and the calibrator lets users manually remap any key to any matrix cell. However, the `ite8258_chassis` **backend protocol constants** are currently ANSI-only.
...
| `docs/developement/backends/ite8258/ite8258-chassis-backend-plan.md` | Mark stages complete, add validation notes |
