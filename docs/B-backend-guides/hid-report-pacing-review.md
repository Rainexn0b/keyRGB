...
| `KEYRGB_<BACKEND>_REPORT_DELAY_MS` | Per-backend override for HID pacing. Backend punctuation is normalized to underscores, so `ite8258-chassis` uses `KEYRGB_ITE8258_CHASSIS_REPORT_DELAY_MS`. Falls back to the global value if unset. |
