...
python3 -m buildpython --run-steps=AppImage
```

Build the AppImage in addition to your selected steps:

```bash
python3 -m buildpython --profile=ci --with-appimage
```

Release profile (includes CI checks + AppImage build):

```bash
python3 -m buildpython --profile=release
```
...
- Behavior:
  ...
  - Excludes `tests/` so integration/hardware tests don’t break CI.
...
### 11) AppImage
- Purpose: build the release artifact `dist/keyrgb-x86_64.AppImage`.
- Output:
  - `dist/keyrgb-x86_64.AppImage`
  - Build working directory under `dist/appimage/`
  - Downloaded tool cached under `dist/tools/`
- Notes:
  - Uses `appimagetool-x86_64.AppImage` (downloaded automatically if missing).
  - Bundles Python dependencies into the AppDir, but currently uses the system `python3` at runtime.
