...
- `src/core/backends/registry.py` uses `_BACKEND_RUNTIME_ERRORS` in both `iter_backends()` and `_probe_backend()`. Broken backend factories and probes are logged and skipped, allowing selection to continue. The paired tests in `tests/core/backends/general/registry/test_backend_registry_unit.py` explicitly check both sides of the contract: `RuntimeError` is contained, while unexpected `AssertionError` still propagates.
