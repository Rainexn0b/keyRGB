...
- `keyrgb/core/backends/`: backend contracts, probing, device opens, protocol helpers, backend policy, and registration
- `keyrgb/core/diagnostics/`: diagnostics JSON, hardware discovery, support/evidence helpers
- `keyrgb/core/effects/`: software and reactive effect engine code
- `keyrgb/core/resources/`: visual layout specs, layout catalog, legend packs, starter keymaps, and reference defaults
- `keyrgb/core/config/` and `keyrgb/core/profile/`: persisted settings, profile storage, defaults, and migrations
- `keyrgb/gui/`: settings, calibrator, per-key editor, reusable widgets, and standalone windows
- `keyrgb/tray/`: startup, lifecycle, controllers, menu building, runtime polling, and tray icon behavior
...
- Keep tray/runtime orchestration in `keyrgb/tray/`; avoid duplicating long-running hardware-control logic inside GUI windows.
...
- Put code in the nearest owner first. Backend logic belongs in `keyrgb/core/backends/`, tray lifecycle and polling in `keyrgb/tray/`, windows in `keyrgb/gui/`, and installer/system integration in `scripts/` or `system/`.
...
- layout IDs and UI labels live in `keyrgb/core/resources/layouts/catalog.py`
- starter keymaps and tweak data live in `keyrgb/core/resources/reference_defaults_specs/`
- per-key overlay and default tweak files live in `keyrgb/core/resources/reference_defaults_specs/per_key_tweaks/`
- legend-pack overrides live in `keyrgb/core/resources/layout_legend_specs.json`

For new visual families:

- declarative visual layout specs live in `keyrgb/core/resources/layout_specs.json`
- rendering and layout-build logic lives in `keyrgb/core/resources/layout.py`
- the reference deck image coordinate space is `BASE_IMAGE_SIZE` in `keyrgb/core/resources/layout.py` (currently `1008x450`)
