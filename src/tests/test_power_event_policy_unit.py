...
def test_event_policy_turns_off_and_restores_when_was_on() -> None:
    ...
    off_res = policy.handle_power_off_event(
        PowerEventInputs(enabled=True, action_enabled=True, is_off=False)
    )
    assert off_res.actions == (TurnOffKeyboard(),)

    restore_res = policy.handle_power_restore_event(
        PowerEventInputs(enabled=True, action_enabled=True, is_off=False)
    )
...
def test_event_policy_does_not_restore_if_already_off() -> None:
    ...
    restore_res = policy.handle_power_restore_event(
        PowerEventInputs(enabled=True, action_enabled=True, is_off=True)
    )
...
def test_event_policy_ignores_events_when_disabled() -> None:
    ...
    res2 = policy.handle_power_restore_event(
        PowerEventInputs(enabled=True, action_enabled=False, is_off=False)
    )
    assert res2.actions == ()
