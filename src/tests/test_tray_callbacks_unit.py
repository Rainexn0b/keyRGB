...
def test_on_effect_clicked_normalizes_and_refreshes() -> None:
    from src.tray.callbacks import on_effect_clicked
...
def test_on_effect_key_clicked_normalizes_key_and_refreshes() -> None:
    from src.tray.callbacks import on_effect_key_clicked

    tray = MagicMock()
    tray._refresh_ui = MagicMock()

    with patch("src.tray.callbacks.apply_effect_selection") as apply:
...
def test_speed_and_brightness_callbacks_delegate() -> None:
    from src.tray.callbacks import on_brightness_clicked_cb, on_speed_clicked_cb
...
def test_on_off_and_on_turn_on_delegate() -> None:
    from src.tray.callbacks import on_off_clicked, on_turn_on_clicked
...
def test_on_hardware_color_clicked_applies_effect_refreshes_and_launches_gui() -> None:
    from src.tray.callbacks import on_hardware_color_clicked
    ...
    with (
        patch("src.tray.callbacks.apply_effect_selection") as apply,
        patch("src.tray.callbacks.launch_uniform_gui") as launch,
    ):
...
def test_on_tcc_profile_clicked_updates_menu_on_success() -> None:
    from src.tray.callbacks import on_tcc_profile_clicked

    tray = MagicMock()
    tray._update_menu = MagicMock()

    with patch("src.tray.callbacks.tcc_power_profiles.set_temp_profile_by_id") as setp:
...
def test_on_tcc_profile_clicked_updates_menu_even_on_failure() -> None:
    ...
    with patch(
        "src.tray.callbacks.tcc_power_profiles.set_temp_profile_by_id",
