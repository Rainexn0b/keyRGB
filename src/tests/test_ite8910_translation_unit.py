...
def test_brightness_speed_report_matches_upstream_reference() -> None:
    assert build_brightness_speed_report_raw(0x00, 0x00).hex() == "cc09000000007f"
    assert build_brightness_speed_report_raw(0x0A, 0x02).hex() == "cc090a0200007f"
    assert build_brightness_speed_report_raw(0x0C, 0x09).hex() == "cc090a0200007f"


@pytest.mark.parametrize(
    ("effect", "expected_hex"),
    [
        (Ite8910Effect.WAVE, "cc00040000007f"),
        (Ite8910Effect.BREATHING, "cc0a000000007f"),
        (Ite8910Effect.SCAN, "cc000a0000007f"),
        (Ite8910Effect.BLINK, "cc0b000000007f"),
        (Ite8910Effect.RANDOM, "cc000900000000"),
        (Ite8910Effect.RIPPLE, "cc070000000000"),
        (Ite8910Effect.SNAKE, "cc000b00000053"),
    ],
)
...
def test_reset_report_matches_upstream_reference() -> None:
    assert build_reset_report().hex() == "cc000c0000007f"


def test_led_color_report_matches_upstream_reference_shape() -> None:
    assert build_led_color_report(0x6E, (0x11, 0x22, 0x33)).hex() == "cc016e1122337f"
...
def test_known_led_ids_match_upstream_c_comment_ranges() -> None:
    assert len(KNOWN_LED_IDS) == 114
    assert KNOWN_LED_IDS[:6] == (0, 1, 2, 3, 4, 5)
    assert 20 not in KNOWN_LED_IDS
    assert 31 not in KNOWN_LED_IDS
    assert 32 in KNOWN_LED_IDS
    assert 44 not in KNOWN_LED_IDS
    assert 45 in KNOWN_LED_IDS
    assert 109 not in KNOWN_LED_IDS
    assert 110 in KNOWN_LED_IDS
    assert 129 not in KNOWN_LED_IDS
    assert 130 in KNOWN_LED_IDS
    assert 168 not in KNOWN_LED_IDS
...
def test_protocol_state_preserves_brightness_and_speed_like_upstream_c() -> None:
    ...
    assert state.set_brightness_raw(0x0A).hex() == "cc090a0000007f"
    assert state.current_brightness_raw == 0x0A
    assert state.current_speed_raw == 0x00

    assert state.set_speed_raw(0x02).hex() == "cc090a0200007f"
    assert state.current_brightness_raw == 0x0A
