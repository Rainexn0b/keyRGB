...
def _known_dormant_action(*, probes: list[dict[str, Any]], usb_key: tuple[int, int] | None) -> str:
    ...
    if usb_key == (0x048D, 0xC197) or "ite8258_perkey_chassis_logo_neon_vent_lenovo_legion" in probe_names:
        return (
            "Known backend scaffold exists for the Lenovo Gen10 composite ITE 8258 path. "
            "Current builds are expected to surface `0x048d:0xc197` through the opt-in experimental `ite8258_perkey_chassis_logo_neon_vent_lenovo_legion` backend, "
