...
calibrated. See `src/gui_keymap_calibrator.py`.
...
def build_layout() -> List[KeyDef]:
    ...
    This is a pragmatic approximation meant for usability.
    """

    # Tuned by eye against the 1008×450 backdrop.
    # The important part is that keys remain inside the deck image and
    # clusters (main, nav, numpad) don't drift.
    unit = 40
    gap = 6

    # Main alphanumeric block origin (top number row)
    x0 = 40
    y0 = 110
    ...
    # Navigation / extra keys block origin (approx)
    nav_x0 = 748

    # Numpad origin (approx)
    nx0 = 806

    keys: List[KeyDef] = []

    # Function row: spaced clusters (Esc) (F1-4) (F5-8) (F9-12)
    fy = r0 - 66
    f_unit = 34
    f_gap = 6
    keys += _units_row_with_spacers(fy, x0, f_unit, f_gap, [
        ("esc", "Esc", 1.05),
        (None, None, 0.45),
    ...
    # Top-right keys vary by Y15 Pro keyboard revision.
    # This variant has a small 3-key cluster: Sc, PrtSc, Del.
    keys += _units_row(fy + 30, nav_x0, 32, 8, [
        ("sc", "Sc", 1), ("prtsc", "PrtSc", 1), ("del", "Del", 1),
    ])

    # Navigation keys for this variant: Home/PgUp/PgDn/End are a row above the numpad.
    keys += _units_row(fy + 30, nx0, 32, 8, [
        ("home", "Home", 1), ("pgup", "PgUp", 1), ("pgdn", "PgDn", 1), ("end", "End", 1),
    ])

    # Number row
    keys += _units_row(r0, x0, unit, gap, [
        ("grave", "`", 1),
    ...
    # Q row
    keys += _units_row(r1, x0, unit, gap, [
        ("tab", "Tab", 1.5),
        ("q", "Q", 1), ("w", "W", 1), ("e", "E", 1), ("r", "R", 1), ("t", "T", 1),
        ("y", "Y", 1), ("u", "U", 1), ("i", "I", 1), ("o", "O", 1), ("p", "P", 1),
        ("lbracket", "[", 1), ("rbracket", "]", 1), ("bslash", "\\", 1.65),
    ])

    # A row
    keys += _units_row(r2, x0, unit, gap, [
        ("caps", "Caps", 1.75),
    ...
    # Z row
    keys += _units_row(r3, x0, unit, gap, [
        ("lshift", "Shift", 2.25),
    ...
    # Bottom row
    keys += _units_row(r4, x0, unit, gap, [
        ("lctrl", "Ctrl", 1.25),
        ("fn", "Fn", 1.0),
        ("lwin", "Win", 1.0),
        ("lalt", "Alt", 1.25),
        ("space", "Space", 6.45),
        ("ralt", "Alt", 1.25),
        # Many newer Y15 Pro variants have a Copilot key in this position.
        ("menu", "Copilot", 1.0),
    ])

    # Arrow keys (placed separately)
    arrow_unit = 34
    ax0 = 642
    ay0 = r4 + 10
    keys += [
    ...
    # Numpad block (approx). Make '+' and 'Enter' tall keys.
    keys += _units_row(r0, nx0, unit, gap, [
        ("numlock", "Num", 1), ("numslash", "/", 1), ("numstar", "*", 1), ("numminus", "-", 1),
    ])
    keys += _units_row(r1, nx0, unit, gap, [
        ("num7", "7", 1), ("num8", "8", 1), ("num9", "9", 1),
    ])
    keys += _units_row(r2, nx0, unit, gap, [
        ("num4", "4", 1), ("num5", "5", 1), ("num6", "6", 1),
    ])
    keys += _units_row(r3, nx0, unit, gap, [
        ("num1", "1", 1), ("num2", "2", 1), ("num3", "3", 1),
    ])
    keys += _units_row(r4, nx0, unit, gap, [
        ("num0", "0", 2.0), ("numdot", ".", 1),
    ])

    # Tall '+' key spans rows r1+r2
    plus_x = nx0 + 3 * (unit + gap)
    plus_y = r1
    plus_h = 2 * unit + gap
    keys.append(KeyDef("numplus", "+", (plus_x, plus_y, unit, plus_h)))

    # Enter key spans rows r3+r4
