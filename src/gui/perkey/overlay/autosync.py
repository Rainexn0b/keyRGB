...
def auto_sync_per_key_overlays(
    *,
    ...
    # Build current geometry in BASE_IMAGE_SIZE coordinates.
    items: list[dict] = []
    for kd in keys:
        x, y, w, h = (float(v) for v in kd.rect)
        gx, gy, gw, gh = apply_global(x, y, w, h)
        x2, y2, w2, h2, cx2, cy2, kt = apply_per_key(kd.key_id, gx, gy, gw, gh)
        items.append(
            {
                "key_id": kd.key_id,
                "gx": gx,
    ...
    # 1) Sync similarly sized keys.
    # Bin by size in BASE coords to avoid over-grouping.
    ...
    for _bin, group in size_bins.items():
        ...
        # Only sync if truly close already ("very similarly sized").
    ...
    # Recompute with updated sizes before snapping rows/cols.
    items2: list[dict] = []
    for kd in keys:
        x, y, w, h = (float(v) for v in kd.rect)
        gx, gy, gw, gh = apply_global(x, y, w, h)
    ...
    # Thresholds based on typical key size.
    med_w = median([float(it["gw"]) for it in items2])
    med_h = median([float(it["gh"]) for it in items2])
    # Use a tighter threshold for X to avoid snapping staggered layouts (e.g. 0.25u offsets).
    x_thresh = max(2.0, med_w * 0.08)
    # Y threshold can be looser to snap rows, but still keep it reasonable.
    y_thresh = max(2.0, med_h * 0.15)

    def cluster_sorted(vals: list[dict], axis: str, thresh: float) -> list[list[dict]]:
        out: list[list[dict]] = []
        cur: list[dict] = []
    ...
    # 2) Snap rows (shared cy) and columns (shared cx).
    for row_group in cluster_sorted(items2, "cy", y_thresh):
        if len(row_group) < 2:
            continue
        target_cy = median([float(it["cy"]) for it in row_group])
        for it in row_group:
            key_id = str(it["key_id"])
            cy = float(it["cy"])
            delta = target_cy - cy
    ...
    # Recompute centers after row snapping for column snapping.
    items3: list[dict] = []
    for kd in keys:
        x, y, w, h = (float(v) for v in kd.rect)
        gx, gy, gw, gh = apply_global(x, y, w, h)
        x2, y2, w2, h2, cx2, cy2, kt = apply_per_key(kd.key_id, gx, gy, gw, gh)
        items3.append({"key_id": kd.key_id, "cx": cx2})

    for col_group in cluster_sorted(items3, "cx", x_thresh):
        if len(col_group) < 2:
            continue
        target_cx = median([float(it["cx"]) for it in col_group])
        for it in col_group:
            key_id = str(it["key_id"])
            cx = float(it["cx"])
            delta = target_cx - cx
