...
- Diagnostics: Include `zoned` in backend capability snapshots and record controller-sleep, screen-dim sync, experimental enablement, and software-effect target in support-bundle config so hardware-confirmation bundles can answer those questions.
...
- Build/Quality: Import the power-mode GUI's `system` module as one name so flattening those fallbacks does not trip the long-import-block gate.
...
- Tray/Power: Keep automatic AC/battery brightness, on/off, idle, scheduler, and hardware-poller transitions from rebuilding the live tray menu. v0.32.0 routed those paths through `_refresh_ui()`, which reintroduced the KDE/AppIndicator `QMenu` host crash that v0.25.5 fixed for profile swaps.
- Tray/Idle: Bind Wayland `wl_seat` / `ext_idle_notifier_v1` at the compositor-advertised version instead of the pywayland maximum, and back off reconnects after repeated failures so a protocol error cannot storm KWin.
...
- Profiles/Storage: Confine reserved `.` / `..` names and give profile JSON, markers, and layout-slot writes the same lock, unique-temp, and fsync contract as config storage.
- Tray/Runtime: Serialize low-frequency lighting transitions in one coordinator and keep pollers that are still alive from being treated as stopped.
- Tray/UI: Render menus from stored snapshots instead of live device probes or cpufreq queries.
- Core/Backends: Fail closed on missing capability evidence, share one backend selection report with diagnostics, and key shared controller state per device.
- Effects: Map software/reactive frames to live backend geometry and replace reactive/hardware-effect introspection with typed contracts.
- Config: Partition settings by domain, keep secondary brightness on `0..100`, and raise when ordinary persistence fails.
- GUI: Move uniform, power-mode, and per-key hardware writes off the Tk callback with generation-aware jobs.
- Installer/Packaging: Match managed udev rules on uninstall, pin AppImage and Actions inputs, and smoke nested resources from built wheel and sdist.
- Build/Quality: Fail continue-on-error runs, architecture warnings, unused runtime symbols, and installer ShellCheck; isolate the default test session from host `XDG_*` and real `/sys` writes.
...
- Settings/Idle-Power: New **Fade duration** slider (0.1–3.0 s, default 0.6 s) for idle dim, turn-off, and restore ramps — the old fixed sub-0.5 s fades completed in a handful of USB-paced frames and read as instant steps.
- Effects/Recovery: After a hidden controller-sleep recovery, the restored scene is now saved as the controller's user mode **by default** (`KEYRGB_RECOVERY_USER_MODE_SAVE=0` opts out), so the firmware's first-keypress wake ramp targets the current scene instead of dropping toward a stale reference while typing.
- Tray/Hardware-Polling: A fresh zero-brightness read now arms a fast 0.25 s confirmation poll, cutting the visible dark blink of a controller-sleep recovery from ~2–4 s to ~0.5–2 s. Transient-0 protection and the recovery circuit breaker are unchanged.
- Effects/Engine: Fix the deck staying dark after lid-open/resume restore for loop/reactive effects — the per-key startup prime now reasserts user mode when the device was explicitly turned off, while keeping the no-flash prime for startup and firmware-sleep recovery.
- Effects/Reactive: Fix a ~2 s deck-wide dip on the first keypress after a wake — the post-restore whole-frame ease now only applies during the actual soft-on ramp; the first-pulse damp still mutes just the pulse.
- Tray/Idle-Power: Idle fades now actually cover reactive per-key effects — turn-off soft-fades instead of cutting to black, and the restore ramp caps reactive base/effect brightness to the fading global level so the configured duration is honored.
- Core/Backends: New backend-declared sleep-state policy seam (`keyrgb_sleep_state_policy`; default `zero_brightness_without_off`, declared on `ite8291r3`) so each backend can own its self-sleep signature. No timeout is hardcoded anywhere.
...
- Tray/Hardware-Poll: Write the recovery cooldown stamp *before* calling tray callbacks (`apply_transition` / `start_current_effect` / `_refresh_ui`) as a defensive ordering guarantee so the stamp persists even when a callback raises or triggers a state-resetting side effect. Log analysis confirmed the previous 5 s cooldown was functional (fires were ~6 s apart = 5 s cooldown + 2 s poll interval), but stamp-first ordering is more robust against future state-owner changes.
- Tray/Hardware-Poll: Seed reactive restore damp before `start_current_effect()` in `_execute_blank_recovery`, mirroring the idle-wake path (`_transition_actions`). Without this, `engine.stop()` inside the restart wiped `_reactive_state` and the first post-restart keystroke flashed at full pulse intensity because the restore damp was gone. The damp seed is scoped to reactive effects and no-ops for everything else.
- Tray/Shutdown: Fix libusb assertion crash (core dump) on Ctrl-C by closing the engine in a `finally` block so cleanup runs even when pystray swallows `KeyboardInterrupt` internally and `icon.run()` returns normally. The original `except KeyboardInterrupt` handler was unreachable because pystray catches the interrupt inside its Xorg main loop. Without engine cleanup, the reactive render thread kept doing USB writes during interpreter teardown, racing with libusb device-handle cleanup and triggering `usbi_mutex_destroy` / `usbi_mutex_lock` C-level assertion failures. The `finally` block calls `engine.close()` (joins the render thread, swaps to `NullKeyboard`, then closes the real device) on every exit path including normal return, Ctrl-C, SIGTERM, and unhandled exceptions.
- Tray/Config-Poll: Fix startup brightness dip (brief off→on flash) by preserving `_last_rendered_brightness` across config-apply effect restarts. Previously, `engine.stop()` inside the restart cleared `_last_rendered_brightness` to None, and the per-frame brightness_guard treated None as 0, causing a visible ramp from 0→8→16→24→32→40 even though the keyboard was already displaying at the target brightness. The fix saves the value before `_start_current_effect()` and restores it after, so the guard sees no brightness delta on the first post-restart frame.
- Testing: Add direct unit coverage for the circuit breaker (consecutive-attempt increment, backoff switch, counter reset), stamp-first ordering (stamp persists when callback raises), reactive restore damp seeding (fires for reactive effects, skipped for non-reactive), engine cleanup on shutdown (Ctrl-C, SIGTERM, pystray-swallows-interrupt, no-op when app is None), and `_last_rendered_brightness` preservation across config-apply restarts.
- Build/Lint: Temporarily pin `ruff>=0.15.10,<0.16` to keep the release gate green. Ruff 0.16 broadened its default rule set (UP/I/RUF/B/SIM now fire by default), producing ~1200 pre-existing violations unrelated to this patch. The pin will be reverted after v0.30.3 so the lint cleanup can be addressed as a focused v0.30.4 effort.

## 0.30.2 (2026-07-20)

- Build/Type Checking (WS4): Extend Step 13 with a green eight-module baseline for pure, non-Tk GUI logic (`perkey` color/dirty-state helpers, overlay geometry, scheduler decisions, draw styling, and support-window geometry) while deliberately skipping imports into Tk-heavy surfaces. Add runner tests and repair annotation-only regressions in the extracted tray effects-menu protocols and tray-icon image alias so the established 407-module runtime check remains green.
- Maintainability/Tests (WS3): Split the 1,275-line support-window megasuite into focused UI/state, export/issue, diagnostics/discovery, and backend-probe modules. Keep all 47 tests while moving the shared Tk-free fake surface to `_support_window_test_fakes.py`; every resulting test module is 315 LOC or less.
- Maintainability/Orchestration (WS2): Extract pure `EffectRoute` classifier (`classify_effect_route`, `should_soft_fade_for_turn_off`, `apply_to_hardware_for_non_reactive`) to `src/tray/pollers/idle_power/_effect_route.py`. Eliminates duplicated `effect in reactive_effects_set` / `sw_effects_set` inline branches across `_execute_turn_off`, `apply_dim_temp_brightness`, and `apply_restore_brightness`.
- Maintainability/Imports: Collapse long import blocks in `hardware_polling.py` (39 → 17) and `_menu_status_devices.py` (34 → 8) via module-alias binding; recovery unit tests import from `hardware._recovery` directly. File-size step no longer reports critical import-block hits.
- Maintainability/Orchestration (WS2): Extract pure power-source idle-guard decisions to `src/tray/pollers/idle_power/_power_source_guard.py` (`plan_power_source_guard_update`, `power_source_idle_guard_active`); wire idle `_runtime` through them with tray-free unit tests.
- Maintainability/Effects: Extract post-restore visual damp helpers from reactive `render.py` into `src/core/effects/reactive/_render_post_restore.py` (397 → 327 LOC); preserve `render.time` monkeypatch seam for tests.
- Maintainability/File-size: Apply `@quality-exception file-size-analysis` waivers to cohesive modules where further splits would add complexity without ownership clarity (chassis protocol/coordinator, sysfs device, Config facade, Tk window surfaces, PowerManager facade, profile storage API, secondary runtime/controller, idle polling entry, buildpython usage graph). File-size step now reports 0 refactor/critical/extreme active hits.
- Maintainability/GUI: Split per-key profile UI actions into `src/gui/perkey/ui/_profile_actions_ui.py` (power-source policy + CRUD); `profile_actions.py` 470 → 242 LOC (WS1 / B1).
- Maintainability/Tray: Extract device-candidate discovery/types from `_menu_status_devices.py` into `src/tray/ui/_menu_status_device_discovery.py`; parent 400 → 291 LOC (WS1 / B3).
- Maintainability/GUI: Move settings window constants / idle-source probe into `src/gui/settings/_settings_window_constants.py`; `window.py` 401 → 377 LOC (WS1 / B2).
- Maintainability/Power: Extract PowerManager battery-saver / AC power-source loop into `src/core/power/management/_manager_battery_saver.py`; `manager.py` 434 → 375 LOC (exits CRITICAL) with monkeypatch seams preserved on the manager module (WS1 / A1).
- Maintainability/Idle-power: Extract Wayland/evdev/logind dim-sensor readers from `_runtime.py` into `src/tray/pollers/idle_power/_runtime_sensors.py`; `_runtime.py` 428 → 314 LOC (WS1 / A7).
- Maintainability/Effects: Extract reactive transition brightness/progress helpers from `_render_brightness.py` into `src/core/effects/reactive/_render_brightness_transition.py` (424 → 304 LOC) (WS1 / A5).
- Maintainability/Tray: Extract `apply_perkey_mode` / `apply_uniform_none_mode` from `_lighting_controller_helpers.py` into `src/tray/controllers/_lighting_mode_apply.py` (414 → 292 LOC); helpers module re-exports for stable imports (WS1 / A6).
- Maintainability/Tray: Split `src/tray/idle_power_state.py` into a thin owner facade plus `src/tray/_idle_power_fields.py` (legacy-attr ↔ owner field bridge) and `src/tray/_idle_power_predicates.py` (forced-off / dim-temp / last-brightness helpers). Facade drops from 497 → 92 LOC; public imports via `idle_power_state` / `protocols` unchanged (WS1 / A2 slice 1; D1/D14 residual).
- Maintainability/Tray: Extract effects/speed/brightness submenu builders from `src/tray/ui/menu.py` into `src/tray/ui/_menu_sections_effects.py` (`build_hw_effects_menu`, `build_sw_effects_menu`, `build_speed_menu`, `build_brightness_menu`). `menu.py` drops from 380 → 297 LOC so `build_menu_items` is a thin orchestrator (WS1 / A8 slice 1; D2 residual).
- Maintainability/Reactive: Extract the reactive-brightness restore-seed queue (`ReactiveRestoreSeed`, `build_reactive_restore_seed`, `queue_reactive_restore_seed`, `apply_queued_reactive_restore_seed`, `apply_reactive_restore_seed`, `seed_reactive_restore_windows`) to `src/core/effects/reactive/_reactive_restore_seed.py` and the low-level atomic transition read/write helpers (`seed_transition_atomic`, `read_transition_atomic`, `clear_transition_atomic`) to `src/core/effects/reactive/_reactive_transition_atomic.py`. `_render_brightness_support.py` drops from 441 → 344 LOC (exits the CRITICAL band) and re-exports the moved names so existing module-attribute access and direct imports keep working (WS1 / A4 slice 1; W2/D14 residual).
- Maintainability/Polling: Extract the hardware-poll blank-recovery group (`_power_source_blank_recovery_eligible`, `_execute_blank_recovery`, `_recover_recent_power_source_blank_best_effort`, `_recover_stable_zero_brightness_best_effort`) and their tightly-coupled logging/refresh/state-query helpers to `src/tray/pollers/hardware/_recovery.py`. `hardware_polling.py` drops from 446 → 231 LOC (out of the REFACTOR band) and re-exports the moved names so existing import paths and monkeypatch seams keep working (WS1 / A3 slice 1; D5/D14 residual).
- Testing/Polling: Add direct unit coverage for the hardware-poll recovery group (22 tests) — previously only exercised indirectly through `_apply_polled_hardware_state`. Covers eligibility gates, callback fallback ordering, recoverable-exception boundaries, hidden-hint cleanup in `finally`, and per-stamp write isolation.
...
- Docs/Screenshots: Refresh the captured tray and GUI screenshots for the current 0.21.1 interface state.
