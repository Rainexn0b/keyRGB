...
if TYPE_CHECKING:
    ...
    from keyrgb.core.utils import exceptions as _exceptions
    from keyrgb.tray.app import backend as _backend, callbacks as _callbacks, lifecycle as _lifecycle
    from keyrgb.tray.controllers import (
        lighting_controller as _lighting_controller,
        software_target_controller as _software_target_controller,
    )
    from keyrgb.tray.integrations import dependencies as _dependencies, runtime as _runtime
    from keyrgb.tray.ui import icon as _icon, menu as _menu, refresh as _refresh
...
class _CallbacksModule(Protocol):
    ...
    def on_selected_device_turn_on_clicked(self, tray: object) -> None: ...
    ...
    def on_perkey_clicked(self, tray: object | None = None) -> None: ...
    ...
    def on_power_mode_settings_clicked(self) -> None: ...
...
def select_backend_with_introspection():
    return _module("keyrgb.tray.app.backend").select_backend_with_introspection()


def select_device_discovery_snapshot():
    return _module("keyrgb.tray.app.backend").select_device_discovery_snapshot()


def load_ite_dimensions():
    return _module("keyrgb.tray.app.backend").load_ite_dimensions()


def apply_brightness_from_power_policy(tray: object, brightness: int) -> None:
    _module("keyrgb.tray.controllers.lighting_controller").apply_brightness_from_power_policy(tray, brightness)


def power_restore(tray: object) -> None:
    _module("keyrgb.tray.controllers.lighting_controller").power_restore(tray)


def power_turn_off(tray: object) -> None:
    _module("keyrgb.tray.controllers.lighting_controller").power_turn_off(tray)


def start_current_effect(tray: object, **kwargs: object) -> bool:
    return bool(_module("keyrgb.tray.controllers.lighting_controller").start_current_effect(tray, **kwargs))


def apply_power_source_perkey_profile_transition(tray: object) -> bool:
    return bool(
        _module("keyrgb.tray.controllers.lighting_controller").apply_power_source_perkey_profile_transition(tray)
    )


def configure_engine_software_targets(tray: object) -> None:
    _module("keyrgb.tray.controllers.software_target_controller").configure_engine_software_targets(tray)


def close_secondary_software_target_cache(tray: object) -> None:
    _module("keyrgb.tray.controllers.software_target_controller").close_secondary_software_target_cache(tray)


def load_tray_dependencies():
    return _module("keyrgb.tray.integrations.dependencies").load_tray_dependencies()


def maybe_autostart_effect(tray: object) -> None:
    _module("keyrgb.tray.app.lifecycle").maybe_autostart_effect(tray)


def start_all_polling(tray: object, *, ite_num_rows: int, ite_num_cols: int) -> None:
    _module("keyrgb.tray.app.lifecycle").start_all_polling(
        tray,
...
def start_power_monitoring(
    ...
    *,
    ...
    return _module("keyrgb.tray.app.lifecycle").start_power_monitoring(
        tray,
...
def update_tray_icon(tray: object, *, animate: bool = True) -> None:
    _module("keyrgb.tray.ui.refresh").update_icon(tray, animate=animate)


def update_tray_menu(tray: object) -> None:
    _module("keyrgb.tray.ui.refresh").update_menu(tray)


def is_permission_denied(exc: BaseException) -> bool:
    return bool(_module("keyrgb.core.utils.exceptions").is_permission_denied(exc))


callbacks: _CallbacksModule = cast(_CallbacksModule, LazyModuleRef("keyrgb.tray.app.callbacks"))
runtime: _RuntimeModule = cast(_RuntimeModule, LazyModuleRef("keyrgb.tray.integrations.runtime"))
icon_mod: _IconModule = cast(_IconModule, LazyModuleRef("keyrgb.tray.ui.icon"))
menu_mod: _MenuModule = cast(_MenuModule, LazyModuleRef("keyrgb.tray.ui.menu"))
