...
from keyrgb.core.backends.ite8910_perkey import protocol as ite8910_protocol
from keyrgb.core.backends.ite8910_perkey.backend import Ite8910Backend
...
ITE8291R3_SPEED_PROBE_KEY = "ite8291r3_speed"
ITE8291R3_SPEED_PROBE_EFFECT = "wave"
ITE8291R3_SPEED_PROBE_UI_SPEEDS: tuple[int, ...] = (1, 3, 5, 7, 10)
...
class _InvertedProbeKeyboard(_ProbeKeyboard):
...
def _build_ite8291r3_speed_probe_plan() -> dict[str, Any]:
    effect_name = ITE8291R3_SPEED_PROBE_EFFECT
    selection_effect_name = hardware_effect_selection_key(effect_name)
    kb = _InvertedProbeKeyboard()
    kb_lock = RLock()
        ...
        samples.append(
            {
                ...
                "payload_speed": payload_speed,
                "raw_speed": raw_speed,
                "raw_speed_hex": f"0x{raw_speed:02x}",
            }
        )
    ...
    return {
        ...
        "observation_prompt": "Which speed steps looked reversed, too close together, or clearly distinct?",
    }
