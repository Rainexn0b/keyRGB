...
def repo_root_from(anchor: str | Path) -> Path:
    ...
    This also supports packaged layouts that keep the importable ``keyrgb/`` tree
    ...
    for parent in candidates:
        try:
            if (parent / "pyproject.toml").exists() and (parent / "keyrgb").exists():
                return parent
        except OSError:
            continue
